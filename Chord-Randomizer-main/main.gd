...
@onready var light_container = $MarginContainer/HBoxContainer/Metronome/MarginContainer/VBoxContainer/Lights/LightContainer
@onready var new_light := preload("res://metronome/light.tscn")
@onready var timer := $Timer
@onready var tempo_button := $MarginContainer/HBoxContainer/Metronome/MarginContainer/VBoxContainer/TimeSignature/VBoxContainer/HBoxContainer2/TempoButton
@onready var click_one := $ClickOne
@onready var click_two := $ClickTwo
@onready var chord_name := $MarginContainer/HBoxContainer/Fingering/VBoxContainer/ChordName
@onready var diagram := $MarginContainer/HBoxContainer/Fingering/VBoxContainer/Diagram
@onready var start_panel := $MarginContainer/HBoxContainer/Metronome/MarginContainer/VBoxContainer/StartPanel
...
func on_new_cycle_started() -> void:
	var random_chord := randi_range(1, 6)
	var fixed_chord
	if mutation_value == 0:
		if first_time:
			first_time = false
...
func on_mutation_changed(new_value) -> void:
	...
	if mutation_value == 0:
		...
		mutation_description.set_text("Repeats one randomly chosen chord progression")
	elif mutation_value == 2:
		mutation_description.set_text("Repeats two randomly chosen chord progressions")
...
func on_variation_changed(new_value) -> void:
	...
	if variation_value == 0:
		...
		variation_description.set_text("Chords anywhere on the neck")
		max_fret = INF
