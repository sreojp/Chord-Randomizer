...
@onready var chord_name_preview := $MarginContainer/HBoxContainer/ChordInfo/VBoxContainer/HBoxContainer/PreviewText
...
@onready var fretboard_preview := $MarginContainer/HBoxContainer/ChordInfo/VBoxContainer/FretboardPreview
...
func _ready():
	...
	Events.connect("new_cycle_started", on_new_cycle_started_preview)
	...
	if mutation_value == 0:
		if first_time:
			...
			if variation_value == 0:
				...
				fretboard_preview.build_chord(chord_to_be_played)
				chord_name_preview.text = "Next Chord: " + chord_to_be_played["name"]
...
func on_new_cycle_started() -> void:
	if mutation_value == 0:
		...
		if index < fixed_chord_progression.size():
				...
				if index + 1 == fixed_chord_progression.size():
					fretboard_preview.build_chord(chord_info.c_major_key_root[fixed_chord_progression[0]])
				else:
					fretboard_preview.build_chord(chord_info.c_major_key_root[fixed_chord_progression[index + 1]])
		else:
			index = 0
			fretboard.build_chord(chord_info.c_major_key_root[fixed_chord_progression[index]])
			chord_name.text = chord_info.c_major_key_root[fixed_chord_progression[index]]["name"]
...
func on_new_cycle_started_preview() -> void:
	if mutation_value == 0:
		if first_time:
			first_time = false
			fixed_chord = random_chord
